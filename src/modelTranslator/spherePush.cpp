...
spherePush::spherePush(int _clutterLevel){

    clutterLevel = _clutterLevel;
    std::string yamlFilePath = "/taskConfigs/spherePushConfig.yaml";
    if(clutterLevel == noClutter){
        yamlFilePath = "/taskConfigs/spherePush.yaml";
    }
    else{
...
void spherePush::generateRandomGoalAndStartState() {
    X_start = returnRandomStartState();
    X_desired = returnRandomGoalState(X_start);
}

MatrixXd spherePush::returnRandomStartState(){
...
MatrixXd spherePush::returnRandomGoalState(MatrixXd X0){
...
std::vector<MatrixXd> spherePush::createInitSetupControls(int horizonLength){
...
std::vector<MatrixXd> spherePush::createInitOptimisationControls(int horizonLength){
...
void spherePush::initControls_mainWayPoints_optimisation(m_point desiredObjectEnd, std::vector<m_point>& mainWayPoints, std::vector<int>& wayPointsTiming, int horizon){
...
std::vector<m_point> spherePush::initControls_createAllWayPoints(std::vector<m_point> mainWayPoints, std::vector<int> wayPointsTiming){
...
std::vector<MatrixXd> spherePush::generate_initControls_fromWayPoints(std::vector<m_point> initPath){
    std::vector<MatrixXd> initControls;
...
bool spherePush::taskComplete(int dataIndex, double &dist){
