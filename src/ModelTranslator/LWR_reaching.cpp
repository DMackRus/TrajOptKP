...
LWRReaching::LWRReaching(): ModelTranslator(){
    std::string yamlFilePath = "/TaskConfigs/free_motion/LWR_reaching.yaml";
    InitModelTranslator(yamlFilePath);
}

bool LWRReaching::TaskComplete(mjData *d, double &dist){
    pose_7 EE_pose;
    MuJoCo_helper->GetBodyPoseQuatViaXpos("swa", EE_pose, d);
...
void LWRReaching::Residuals(mjData *d, MatrixXd &residuals){
    int resid_index = 0;
...
//                                       + pow(diff_z, 2));
    ...
    MuJoCo_helper->GetRobotJointsPositions("LWR", joint_positions, d);
    for(int i = 0; i < joint_positions.size(); i++){
        residuals(resid_index++, 0) = joint_positions[i] - residual_list[0].target[i];
    }

    std::vector<double> joint_velocities;
    MuJoCo_helper->GetRobotJointsVelocities("LWR", joint_velocities, d);
...
void LWRReaching::SetGoalVisuals(mjData *d){
    ...
    MuJoCo_helper->SetRobotJointPositions("LWR", joint_positions, d);
    ...
    pose_6 EE_pose;
    MuJoCo_helper->GetBodyPoseAngleViaXpos("swa", EE_pose, d);
...
void LWRReaching::ReturnRandomStartState(){
...
void LWRReaching::ReturnRandomGoalState(){
...
std::vector<MatrixXd> LWRReaching::CreateInitOptimisationControls(int horizonLength){
